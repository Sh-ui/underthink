...
Underthink is a Claude Code workflow built around a Sonnet 5 host running at
medium effort. The host keeps the project in view, routes decisions, checks
evidence, and moves the run forward without trying to be the smartest model in
the room.
...
```mermaid
flowchart TB
    User["Project direction"] --> Host["Sonnet 5 host — medium"]
    ...
    Planner --> Builder["Sonnet 5 builders — medium"]
    Architect --> Host
    Auditor --> Host
    Crawler --> Host
    Builder --> Host
...
| Role | What it owns |
| --- | --- |
| `underthink:host` | Main thread, dispatch, evidence checks, user communication |
...
The plugin activates `underthink:host` as Claude Code's main agent while the
plugin is enabled. That pins the main thread to Sonnet 5 at medium effort.
Outside an `/underthink` invocation, the host behaves normally and does not
force the workflow onto ordinary requests.

Disable or uninstall the plugin to return to your previous default agent.
...
The package contains prompts and default agent settings. It installs no hooks,
executables, MCP servers, network services, or runtime dependencies. Claude
Code's normal tool permissions still apply.
